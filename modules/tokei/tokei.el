...
(defun tokei-exec-callback (statistics buffer)
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
      (setq tabulated-list-entries
            (cl-loop for (lang . data) in statistics
                     for files = (length (cdr (assoc "reports" data)))
                     for code = (cdr (assoc "code" data))
                     for comments = (cdr (assoc "comments" data))
                     for blanks = (cdr (assoc "blanks" data))
                     for lines = (+ code comments blanks)
                     unless (equal lang "Total")
                     summing files into total-files
                     and summing code into total-code
                     and summing comments into total-comments
                     and summing blanks into total-blanks
                     and summing lines into total-lines
                     and collecting
                     (list lang files code comments blanks lines)
                     into entries
                     end
                     finally return
                     (append
                      (mapcar
                       (lambda (entry)
                         (pcase-let ((`(,lang ,files ,code ,comments ,blanks ,lines) entry))
...
(defun tokei-mode-refresh ()
  ...
  (tokei-exec tokei-paths #'tokei-exec-callback (current-buffer)))
...
\\{tokei-mode-map}"
  (setq tabulated-list-format
        `[("Language" 25 ,(tokei-make-string-comparer 0))
          ("Files" 10 ,(tokei-make-number-comparer 1) :right-align t)
          ("Code" 10 ,(tokei-make-number-comparer 2) :right-align t)
          ("Comments" 10 ,(tokei-make-number-comparer 3) :right-align t)
          ("Blanks" 10 ,(tokei-make-number-comparer 4) :right-align t)
          ("Lines" 10 ,(tokei-make-number-comparer 5) :right-align t)])
